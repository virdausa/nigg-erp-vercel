...
$source_query = "SELECT CUSTOMER_NO, CUSTOMER_NAME, CUSTOMER_STATE, CONTACT FROM customers";
...
if ($source_result->num_rows > 0) {
    while ($row = $source_result->fetch_assoc()) {
        ...
        $destination_query = $destination_conn->prepare(
            "INSERT INTO customers (id, name, email, phone_number, status) VALUES (?, ?, ?, ?, ?)
            ON DUPLICATE KEY UPDATE name = VALUES(name), email = VALUES(email), phone_number = VALUES(phone_number), status = VALUES(status)"
        );

        $destination_query->bind_param("issss", $id, $name, $email, $phone_number, $status);
